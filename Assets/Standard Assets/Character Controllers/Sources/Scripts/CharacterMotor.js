...
var spinAttackFlag : boolean = false;
...
private function UpdateFunction () {
	...
	CharacterMotorAttack(); //Check if attacking.
}
...
private function CharacterMotorAttack(){
	if (Input.GetKeyDown("q") && !(timer >0)) {
			timer = 0.5;
			spinAttackFlag = true;
	}
	if (timer > 0) {
		...
		if (timer <= 0){
			spinAttackFlag = false;
		}
	}
	//Debug.Log("Spinning? " + spinAttackFlag);
